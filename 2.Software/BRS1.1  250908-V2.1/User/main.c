...
int main(void)
{
    ...
    // 3. 启动定时器 (核心心跳)
    // 假设 TIMx_Configuration 配置为 1ms 中断
	TIMx_Configuration(); 
	...
	while(1)
	{	
        // 等待 1ms 时间片到来 (非阻塞等待期间 CPU 空转，或可进入 Sleep 模式)
        if (g_system_tick)
        {
            ...
            // --- E. 通信发送 (Communication) ---
            // 在 1ms 内尽量发送所有电机数据。
            // 假设波特率足够高 (如 3Mbps+)，一次性发送所有指令以保证同步性
            for(int k=0; k<8; k++) {
